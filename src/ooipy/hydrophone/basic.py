...
class HydrophoneData(Trace):
    ...
    def frequency_calibration(self, N):
        ...
        filename = (
            os.path.dirname(ooipy.__file__) + "/hydrophone/calibration_by_assetID.csv"
        )
        ...
        if avg_time is None:
            ...
            if len(self.data[int((nbins - 1) * fs * avg_time) :]) >= L:  # noqa
                ...
                if len(Pxx) != int(L / 2) + 1:
                    if verbose:
                        print(
                            "Error while computing Welch " "estimate residual segment"
                        )
                    self.spectrogram = None
                    return None
                else:
        ...
        with mp.get_context("spawn").Pool(n_process) as p:
            try:
                ...
                spec_xr = xr.DataArray(
                    ...
                    dims=["time", "frequency"],
                    coords={
                        "time": np.array(time_specgram),
                        "frequency": np.array(specgram_list[0].freq),
                    },
                    attrs=dict(
                        start_time=self.stats.starttime.datetime,
                        end_time=self.stats.endtime.datetime,
    ...
    def wav_write(self, filename, norm=False, new_sample_rate=None):
        ...
        if new_sample_rate is None:
            ...
            if new_sample_rate > self.stats.sampling_rate:
                ...
                warnings.warn(
                    "New sample rate is same as original data. " "No resampling done."
                )
                sampling_rate = self.stats.sampling_rate
            elif new_sample_rate < self.stats.sampling_rate:
                warnings.warn(
...
class Spectrogram:
    ...
    def visualize(
        ...
        if len(self.time) != len(self.values):
            t = np.linspace(
                0, len(self.values) - 1, int(len(self.values) / res_reduction_time)
            )
        else:
